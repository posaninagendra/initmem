...
static struct plugin_info initmem_plugin_info = {"1.0", "This plugin initialises uninitialised variables of a given C++ program"};

namespace{
	const pass_data initmem_pass_data = {
		GIMPLE_PASS,
		"initmem_pass", /* name */
	...
	struct initmem_pass : gimple_opt_pass{
		private: 
			std::set<tree> uninitialised_vars;
		
		public:
		/* Class constructor */
		initmem_pass(gcc::context *ctx) 
			: gimple_opt_pass(initmem_pass_data, ctx){}
		...
		virtual initmem_pass *clone() override { return this; }
		...
		static tree vars_callback_stmt(gimple_stmt_iterator *gsi, 
			...
			*handled_all_ops = true;
			...
			initmem_pass *this_ = reinterpret_cast<initmem_pass *>(wi->info);
...
int plugin_init(struct plugin_name_args *plugin_info, struct plugin_gcc_version *version){
	...
	register_callback(plugin_info->base_name,
			/* event */PLUGIN_INFO,
			/* callback */	NULL,
			/* userdata */ &initmem_plugin_info);

	struct register_pass_info pass_info;

	pass_info.pass = new initmem_pass(g);
